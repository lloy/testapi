...
"""
    ...
    Recommendvd ways to use sessions within this framework:
"""
...
from rosmanager.common import db_models as models
from rosmanager.common import exception as exc
...
class Connection(EngineFacade):
    ...
    def get_site(self, name):
        ...
        try:
            ...
            site = session.query(models.Site).\
                options(subqueryload_all(models.Site.route)).\
    ...
    def list_pod(self, **kwargs):
        ...
        try:
            session = self.engine.get_session()
            if not kwargs:
                return session.query(models.Pod).options(joinedload_all('*')).all()
            if kwargs.keys()[0] not in _support:
                raise exc.ErrorKwargs('kwargs error in list_pod')
            return session.query(models.Pod).options(joinedload_all('*')).\
    ...
    def list_app(self, **kwargs):
        ...
        try:
            session = self.engine.get_session()
            if not kwargs:
                return session.query(models.App).options(joinedload_all('*')).all()
            if kwargs.keys()[0] not in _support:
                raise exc.ErrorKwargs("kwargs error in list_app")
            return session.query(models.App).options(joinedload_all('*')).\
    ...
    def list_clusters_from_app(self, app_id):
        try:
            ...
            clusters = session.query(models.Cluster).\
                options(joinedload_all(models.Cluster.pod, models.Cluster.datastore)).\
    ...
    def get_subinterface_from_route(self, route_id, **kwargs):
        _support = ('status',)
        try:
            session = self.engine.get_session()
            if not kwargs:
            ...
            if _support[0] not in kwargs:
                raise exc.NotFoundKey("not support %s in subinterface" % _support[0])
            interfaces = session.query(models.Interface).\
                filter(models.Interface.route_id == route_id).all()
            # there is only a interface in per route
            interface = interfaces[0]
            subinterface = session.query(models.Subinterface).\
                options(joinedload_all('*')).\
                filter(models.Subinterface.interface_id == interface.interface_id).\
                filter(models.Subinterface.status == kwargs['status']).first()
            return subinterface
        except:
            raise
        finally:
    ...
    def alloc_vlan(self, **kwargs):
        """
            kw = {'app_id': xx,
                  ...
                  'status': 'processing',
                  'sub_net': {
                                'network_num': '172.2.3.1/24',
                                'network_address': '172.2.3.1',
                                'level': 'primary',
                                'step': 'processing'
                             }
                }
        ...
        try:
            ...
            subinterface.alloc_time = timeutils.utcnow()
            if sub_net:
    ...
    def update_vlan(self, subinterface_id, **kwargs):
        ...
        delete:
            kw = {'qos': 20,
                  'sub_net': {'network_address': '172.3.2.1/24',
                              'op_type': 'delete'}
    ...
    def alloc_gic(self, **kwargs):
        try:
            ...
            gic.alloc_time = timeutils.utcnow()
            gic.qos = kwargs['qos']
    ...
    def free_gic(self, gic_id):
        try:
            kwargs = {'alloc_time': None, 'qos': None, 'customer_id': None, 'status': None}
            session = self.engine.get_session()
    ...
    def list_gicextension_from_route(self, route_id, **kwargs):
        ...
        try:
            ...
            for gicextension in gicextensions:
                ...
                subinterface = session.query(models.Subinterface).\
                    options(joinedload_all('*')).\
                    filter(models.Subinterface.subinterface_id == gicextension.subinterface_id).\
                    one()
                print subinterface.subinterface_name
                print subinterface.interface.route_id
                print route_id
    ...
    def list_vm_from_serial(self, **kwargs):
        ...
        try:
            ...
            if _support[2] in kwargs:
                return session.query(models.Serial_Connection.vm_name, models.Serial_Connection.port).\
                    options(joinedload_all('*')).\
    ...
    def delete_vm_serial(self, vm_name):
        ...
        try:
            session = self.engine.get_session()
            session.query(models.Serial_Connection).\
                filter(models.Serial_Connection.vm_name == vm_name).delete()
            session.commit()
        except NoResultFound:
    ...
    def update_vm_serial(self, vm_name, **kwargs):
        try:
            ...
            q = session.query(models.Serial_Connection).\
                filter(models.Serial_Connection.vm_name == vm_name).first()
            q.update(kwargs)
            session.commit()
        except NoResultFound:
            raise exc.NoResultFound('not found vm')
    ...
    def list_configure_step_isok(self, site_name):
        try:
            session = self.engine.get_session()
            return session.query(models.Vm).\
                filter(models.Vm.site_name == site_name).\
                filter(or_(models.Vm.status == 'running',
                           models.Vm.status == 'stop')).\
                filter(models.Vm.configure_step == 'ok').all()
        except:
            raise
        finally:
            session.close()
    ...
    def list_vming_from_site(self, site_name):
        try:
            ...
            return session.query(models.Vm).\
                options(subqueryload_all('*')).\
                filter(models.Vm.site_name == site_name).\
                filter(models.Vm.status != 'deleted').\
    ...
    def list_vm(self, **kwargs):
        ...
        try:
            ...
            if _support[0] in kwargs:
                return session.query(models.Vm).\
                    options(joinedload_all('*')).\
                    filter(models.Vm.site_name == kwargs['site_name']).all()
            if _support[1] in kwargs:
                return session.query(models.Vm).\
                    options(joinedload_all('*')).\
                    filter(models.Vm.pod_name == kwargs['pod_name']).all()
            if _support[2] in kwargs:
                return session.query(models.Vm).\
                    options(joinedload_all('*')).\
                    filter(models.Vm.cluster_name == kwargs['cluster_name']).all()
            if _support[3] in kwargs:
                return session.query(models.Vm).\
                    options(joinedload_all('*')).\
                    filter(models.Vm.status == kwargs['status']).all()
            if _support[4] in kwargs:
                return session.query(models.Vm).\
                    options(joinedload_all('*')).\
                    filter(models.Vm.configure_step == kwargs['configure_step']).all()
            if _support[5] in kwargs:
                return session.query(models.Vm).\
                    options(joinedload_all('*')).\
                    filter(models.Vm.customer_id == kwargs['customer_id']).all()
            if _support[6] in kwargs:
                return session.query(models.Vm).\
                    options(joinedload_all('*')).\
    ...
    def update_vm_processing(self, vm_id, **kwargs):
        try:
            session = self.engine.get_session()
            vm = session.query(models.Vm).filter(models.Vm.vm_id == vm_id).one()
            vm.processing = kwargs['processing']
            session.commit()
        except NoResultFound:
            raise exc.NoResultFound('not found vm')
    ...
    def update_vm(self, vm_id, **kwargs):
        ...
        try:
            ...
            if kwargs.get('nic_status', None):
                nic_status = kwargs['nic_status']
    ...
    def list_deleted_vms(self):
        try:
            session = self.engine.get_session()
            deleted_vms = session.query(models.Vm).\
                filter(models.Vm.status == 'deleted').\
                filter(models.Vm.configure_step == 'deleted').all()
            return deleted_vms
        except Exception:
            return []
        finally:
    ...
    def delete_vm(self, vm_id):
        try:
            session = self.engine.get_session()
            session.query(models.Vm).filter(models.Vm.vm_id == vm_id).delete()
            session.commit()
        except NoResultFound:
            raise exc.NoResultFound('not found nic')
